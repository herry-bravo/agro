            ...
            <div class="table-responsive">
                <table id="table" class="table w-100">
                    ...
                    <tbody>
                        ...
                        {{-- DATA 1 --}}
                        @foreach ($data as $row)
                            @php $selisih = $row->entered_dr - $row->entered_cr; @endphp
                            @if ($selisih != 0)
                                <tr>
                                    ...
                                    <td class="text-end"><b>{{ number_format($selisih, 3) }}</b></td>
                                </tr>
                                @php
                                ...
                                @foreach ($lines as $line)
                                    @if ($line->code_combination_id == $row->code_combination_id)
                                        @php $selisihLine = $line->entered_dr - $line->entered_cr; @endphp
                                        @if ($selisihLine != 0)
                                            <tr>
                                                ...
                                                <td class="text-end">{{ number_format($selisihLine, 3) }}</td>
                                            </tr>
                                        @endif
                                    @endif
                                @endforeach
                        ...
                        @foreach ($data2 as $row)
                            @php $selisih = $row->entered_cr - $row->entered_dr; @endphp
                            @if ($selisih != 0)
                                <tr>
                                    <td width="auto"><b>{{ $row->code_combination_id }} {{ $row->coa->description }}</b></td>
                                    <td></td><td></td><td></td><td></td>
                                ...
                                @foreach ($lines2 as $line)
                                    @if ($line->code_combination_id == $row->code_combination_id)
                                        @php $selisihLine = $line->entered_cr - $line->entered_dr; @endphp
                                        @if ($selisihLine != 0)
                                            <tr>
                                                <td width="auto">&emsp; {{ $line->gl->name }}</td>
                                                <td>{{ $line->effective_date->format('d/m/y') }}</td>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th class="text-end">{{ number_format($subtotal_dr - $subtotal_cr, 2) }}</th>
                        </tr>
                    </tfoot>
