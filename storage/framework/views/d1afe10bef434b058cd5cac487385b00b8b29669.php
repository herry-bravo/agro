...
<?php $__env->startPush('script'); ?>
    ...
    <script>
        document.getElementById('printStruk').addEventListener('click', function (e) {
            ...
            // Data toko
            const storeName = "SURYA AGRO";
            const storeAddress = "Store Address, City, ZIP";
            ...
            // Informasi transaksi
            doc.text(cashierName, margin, y);
            y += 3;
            doc.text('Transaction: ' + transactionId, margin, y);
            y += 3;
            doc.text('Date: ' + purchaseDate, margin, y);
            y += 4;
            ...
            items.forEach((item, index) => {
                ...
                if (y + rowHeight > maxHeight) {
                    doc.addPage();
                    y = 10; // Reset posisi ke atas halaman baru
                }
                ...
                doc.text(": " + item.qty, margin + 15, y);
                y += 4;

                doc.text("Price", margin, y);
                doc.text(": " + item.price, margin + 15, y);
                y += 6;
            });
        ...
        document.getElementById('printButton').addEventListener('click', function(e) {
            ...
            const { jsPDF } = window.jspdf;
            const doc = new jsPDF('p', 'mm', 'a4'); // Ukuran A4 dengan orientasi potrait
            ...
            const invoiceNumber = "<?php echo e($sales->inv_number); ?>"; // Ganti dengan nomor faktur yang diinginkan
            const soNumber = "<?php echo e($sales->order_number); ?>";
            const invDate = "<?php echo e($sales->inv_date); ?>";
            const taxDate = "<?php echo e($sales->tax_date); ?>"; // Ganti dengan tanggal pajak yang diinginkan
            const total = "<?php echo e(number_format($total)); ?>"; // Ganti dengan tanggal pajak yang diinginkan

            // Fungsi untuk mengubah format tanggal menjadi d-m-Y
            ...
            // Menambahkan nomor invoice di sebelah kanan
            doc.setFontSize(12);
            doc.text('' + invoiceNumber, 170, 20); // Posisi X=150 untuk menempatkan di sebelah kanan
            ...
            doc.text('Address: ' + customerAddress, margin, 50);

            // Menambahkan ruang
            doc.text('Tax Date: ' + formattedTaxDate, margin, 70);
            doc.text('Invoice Date: ' + formattedInvDate, margin, 80);

            // Menambahkan garis pembatas bawah
            doc.line(margin, 95, 200, 95);

            // Tabel untuk item invoice
            const tableTop = 100;
            const tableHeaders = ['Item', 'Quantity', 'Unit Price', 'Sub Total'];

            const tableData = [
                <?php $__currentLoopData = $so_detil; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    [
                        "<?php echo e(substr($row->user_description_item ?? '', 0, 20)); ?>...",
                        "<?php echo e(number_format($row->ordered_quantity ?? '')); ?>",
                        "<?php echo e(number_format($row->unit_selling_price ?? '')); ?>",
                        "<?php echo e(number_format($row->unit_list_price ?? '')); ?>"
                    ],
            ...
            tableHeaders.forEach((header, index) => {
                doc.text(header, margin + index * 40, y);
            });
            ...
            tableData.forEach(row => {
                row.forEach((cell, index) => {
                    doc.text(cell, margin + index * 40, y);
                });
                y += 10;
            });

            // Menambahkan total akhir
            doc.text('Total: ' + total, margin + 160, y + 10);

            // Menambahkan nama perusahaan di kiri footer
            const companyName = "SURYA AGRO"; // Ganti dengan nama perusahaan Anda
            doc.setFont("helvetica", "bold");
            doc.text(companyName, margin, y + 20); // Nama perusahaan di sisi kiri

            // Menambahkan waktu pembelian di kanan footer
            const purchaseTime = new Date().toLocaleString(); // Menampilkan waktu pembelian saat ini
            const pageWidth = doc.internal.pageSize.width;
            const purchaseTimeWidth = doc.getStringUnitWidth(purchaseTime) * doc.internal.getFontSize();
            const xPosition = pageWidth - purchaseTimeWidth - margin; // Posisi kanan
            doc.text(purchaseTime, xPosition, y + 20); // Waktu pembelian di sisi kanan
